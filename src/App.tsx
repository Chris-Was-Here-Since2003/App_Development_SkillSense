...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#F3F4F6] text-[#111827] flex flex-col justify-between" id="app-root">
      ...
      {/* Views Port */}
      <main className="flex-1 w-full px-0.5 py-0.5 sm:px-1.5 sm:py-1.5 lg:px-2 lg:py-2 xl:px-3 xl:py-2">
        {isLoading ? (
          <LoadingState />
        ) : error ? (
            ...
            {currentTab === "home" ? (
              ...
              <div className="w-full px-2 sm:px-4 lg:px-6 py-3 sm:py-4 lg:py-6">
                <div className="max-w-7xl mx-auto space-y-2">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-2.5 sm:p-3 rounded-xl border border-slate-200 gap-2 shadow-sm">
                    <div>
                      <h2 className="text-lg sm:text-xl font-bold tracking-tight text-slate-900 flex items-center gap-2 mb-1">
              ...
              <div className="w-full px-1 sm:px-2 lg:px-3 py-1 sm:py-2 lg:py-2">
                <ResumeGenerator analysisResult={analysisResult} />
              </div>
            ) : (
              <div className="max-w-[1380px] mx-auto px-1 sm:px-2 md:px-3 py-1 sm:py-2 md:py-2">
                {analysisResult ? (
                  <div className="space-y-2">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-2.5 rounded-lg border border-slate-200 gap-2">
                      <div>
                        <h2 className="text-base font-bold text-slate-900">Career Diagnostics</h2>
                        <p className="text-[11px] text-slate-500">Interactive strategic labor model assessment</p>
                      </div>
                      <div className="flex gap-2">
                        <button
                          onClick={handleReset}
                          className="bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-semibold px-3 py-1.5 rounded-lg text-[11px] transition-colors cursor-pointer"
                        >
                          Clear & Upload New
                        </button>
                        ...
                        {currentUser ? (
                          isCurrentResumeSaved ? (
                            ...
                            <button
                              ...
                              className="inline-flex items-center gap-1.5 text-[11px] font-bold text-blue-700 bg-blue-50 hover:bg-blue-100 border border-blue-200 px-2.5 py-1.5 rounded-lg transition-all cursor-pointer disabled:opacity-50"
                            >
                          ...
                          <button
                            ...
                            className="inline-flex items-center gap-1 bg-slate-50 hover:bg-slate-100 text-slate-600 text-[11px] font-semibold px-2.5 py-1.5 rounded-lg border border-slate-200 cursor-pointer"
                          >
                  ...
                  <div className="py-4 bg-white border border-slate-200 rounded-3xl shadow-sm p-4 md:p-8">
